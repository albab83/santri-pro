              ...
              <div class="flex items-start justify-between mb-4">
                <div class="flex items-center space-x-3">
                  <div class="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center">
                    <span class="text-sm font-semibold text-blue-600">#{journals.length - index}</span>
                  </div>
                  <div>
                    <p class="text-sm font-medium text-gray-900">{formatDate(journal.created_at)}</p>
                    {#if journal.status}
                      <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {getStatusColor(journal.status)} mt-1">
                        {journal.status}
                      </span>
                    {/if}
                  </div>
