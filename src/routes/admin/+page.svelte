<script lang="ts">
  ...
  $: stats = {
    ...
    projectDiterima: projects.filter(p => p.status.toLowerCase() === 'diterima' || p.status.toLowerCase() === 'approved').length,
    projectMenunggu: projects.filter(p => p.status.toLowerCase() === 'menunggu' || p.status.toLowerCase() === 'pending').length,
    projectDitolak: projects.filter(p => p.status.toLowerCase() === 'ditolak' || p.status.toLowerCase() === 'rejected').length
  };
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center space-x-2">
          <button
            ...
            class="bg-blue-50 hover:bg-blue-100 text-blue-600 font-medium px-4 py-2 rounded-lg transition-colors duration-200 flex items-center space-x-2"
          >
          ...
          <div class="relative">
            <svg width="200" height="200" viewBox="0 0 200 200" class="transform -rotate-90">
              {#each chartData as item, i}
                ...
                {@const prevPercentages = chartData.slice(0, i).reduce((sum, prev) => sum + (prev.value / total) * 100, 0)}
            ...
            {#each chartData as item}
              <div class="flex items-center space-x-3">
                ...
                <span class="text-sm text-gray-500">
                  {stats.totalProject > 0 ? Math.round((item.value / stats.totalProject) * 100) : 0}%
                </span>
              </div>
            {/each}
          </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl p-8">
      ...
      {#if projects.filter(p => p.status === 'menunggu').length === 0}
        <div class="text-center py-12">
          ...
          <h3 class="text-lg font-medium text-gray-900 mb-2">Tidak ada project yang menunggu persetujuan</h3>
